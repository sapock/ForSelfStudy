...
function StatsScreen({ subjects, streakDays, totals }) {
  // generate 100-cell heatmap (last 100 days mock)
  const heat = useMemoS(() => {
    const arr = [];
    for (let i = 0; i < 100; i++) {
  ...
  return (
    <div className="main-inner">
      ...
      <div className="stats-grid">
        ...
        <KPICard label="평균 정답률" value={`${totals.totalAttempts > 0 ? Math.round(totals.totalCorrect/totals.totalAttempts*100) : 0}%`}
                 delta="+4%p" deltaDir="up" />
      ...
      <div className="tile" style={{ marginBottom: 20 }}>
        <div className="row between">
          ...
          <div className="row" style={{ gap: 8, fontSize: 11, color: "var(--gray-500)" }}>
            <span>적음</span>
            <span className="heat-cell" style={{ width: 12, height: 12 }} />
            <span className="heat-cell l1" style={{ width: 12, height: 12 }} />
            <span className="heat-cell l2" style={{ width: 12, height: 12 }} />
            <span className="heat-cell l3" style={{ width: 12, height: 12 }} />
            <span className="heat-cell l4" style={{ width: 12, height: 12 }} />
            <span>많음</span>
          </div>
        </div>
