/* Studia — 학습 자료 mock data */
/* 한 과목(subject)은 여러 챕터(chapter)를 갖고,
   각 챕터는 항목(item: term/definition pair)들을 갖는다. */

const NOW = Date.now();
const DAY = 86400000;

function mk(term, def, opts = {}) {
  return {
    id: opts.id || crypto.randomUUID().slice(0, 8),
    term, def,
    starred: !!opts.starred,
    mastered: opts.mastered || 0,      // 0 = 미학습, 1 = 학습중, 2 = 완료
  ...
  },
  {
    ...
    chapters: [
      ...
      },
      {
        ...
        name: "Part 7 비즈니스 어휘",
        items: [
          mk("invoice", "n. 송장, 청구서 — settle the invoice 청구서를 정산하다.", { mastered: 1, correct: 1, wrong: 0, starred: true }),
          mk("reimbursement", "n. 환급, 상환.", { mastered: 0 }),
          mk("itinerary", "n. 여행 일정표.", { mastered: 0 }),
          mk("subsidiary", "n. 자회사.", { mastered: 0 }),
        ],
      },
    ],
  },
...
window.STREAK_DAYS = STREAK_DAYS;
